...
from aakaar.storage import ChromaVectorStore, VectorItem
...
def test_upsert_and_search(tmp_path: Path) -> None:
    store = ChromaVectorStore(tmp_path, dim=3)
    items = [
...
def test_delete(tmp_path: Path) -> None:
    store = ChromaVectorStore(tmp_path, dim=2)
    store.upsert(
        "t1",
        "caps",
        [
...
def test_persists_across_instances(tmp_path: Path) -> None:
    store1 = ChromaVectorStore(tmp_path, dim=2)
    store1.upsert("t1", "caps", [VectorItem(id="a", vector=_vec(1, 0), payload={"x": 1})])

    store2 = ChromaVectorStore(tmp_path, dim=2)
