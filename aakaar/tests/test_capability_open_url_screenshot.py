...
from aakaar.capabilities.screenshot import CAP_REF as SHOT_REF
    ...
    dag = Dag(
        nodes=[
            Node(
                ...
                inputs={"url": "https://aakaar.test/dashboard"},
            )
        ]
    )
...
async def test_open_url_then_screenshot_chain(tmp_path: Path) -> None:
    ...
    dag = Dag(
        nodes=[
            Node(
                id="open", kind=NodeKind.CAPABILITY, ref=OPEN_REF,
                inputs={"url": "https://aakaar.test/report"},
                outputs_as="open",
            ),
            Node(
    ...
    assert image_uri.startswith("aakaar://")
