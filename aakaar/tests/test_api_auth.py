...
def test_login_success(deps: AppDependencies, client: TestClient) -> None:
    seed_superuser(deps, email="su@aakaar.test", password="hunter22-correct")
    r = client.post(
        "/auth/login",
        json={"email": "su@aakaar.test", "password": "hunter22-correct"},
    )
    assert r.status_code == 200
    body = r.json()
...
def test_login_wrong_password(deps: AppDependencies, client: TestClient) -> None:
    ...
    r = client.post(
        "/auth/login",
        json={"email": "su@aakaar.test", "password": "wrong"},
    )
...
def test_login_unknown_email(client: TestClient) -> None:
    r = client.post(
        "/auth/login",
        json={"email": "nope@aakaar.test", "password": "whatever"},
    )
    assert r.status_code == 401
...
def test_superuser_creates_tenant_then_admin_logs_in(
    ...
    seed_superuser(deps, email="su@aakaar.test", password="hunter22-correct")
    su_token = login(client, email="su@aakaar.test", password="hunter22-correct")
