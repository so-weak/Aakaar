"""Add CTSBackImageHarvest v2 — multi-batch sweep, NO waits.

Sweeps a dates/cycles/batches JSON via cap.cts_back_image_sweep (download every
batch's back images into ONE consolidated ZIP with per-batch subfolders), with no
control.wait nodes and delay_seconds=0.

    cd aakaar && .venv/bin/python -m aakaar.scripts.seed_cts_back_image_harvest_v2
"""
...
_ART = Path(__file__).resolve().parents[3] / "docs" / "design" / "ctsbackimageharvest-v2-dag.json"

# EDIT for the real run (dates -> cycles -> batch numbers).
_BATCHES = [
...
_NODES: list[tuple[str, NodeKind, str, dict]] = [
    ...
    ("back_image_sweep", NodeKind.CAPABILITY, "cap.cts_back_image_sweep",
     {"session": _SES, "batches": _BATCHES, "record_type": "TXN", "core_system": "FLEX",
      "delay_seconds": 0, "zip_filename": "back_images_consolidated.zip"}),
...
def main() -> int:
    ...
    with factory.session() as s:
        ...
        print(f"DAG valid for {_TENANT_SLUG}: {len(dag.nodes)} nodes, {len(dag.edges)} edges (no waits, sweep)")
        ...
        for v in range(1, wf.latest_version + 1):
            ...
            if ver and any(n.get("ref") == "cap.cts_back_image_sweep" for n in ver.dag.get("nodes", [])):
                print(f"{_NAME}: v{v} already uses cap.cts_back_image_sweep; nothing to do.")
                return 0
        new = workflows_repo.add_version(
            s, tenant_id=tenant.id, workflow_id=wf.id, created_by=admin.id, dag=dag,
            rationale="v2: multi-batch back-image sweep -> consolidated zip; no waits.")
        s.commit()
