...
class LLMClient(Protocol):
    ...
    def complete_text(self, system: str, user: str) -> str:
        ...
        Capabilities (web_scrape, email_parse, doc_extract) use this to turn
        unstructured content into structured data: they pass their own system
        + user prompts and parse the returned text themselves (usually JSON).
        Distinct from the planner calls — capabilities must never use it for
        action selection, only for parsing/extraction."""
        ...
...
class FakeLLMClient:
    ...
    def complete_text(self, system: str, user: str) -> str:
        # Records the call and returns a primed reply if one is queued,
        # otherwise an empty string so capabilities fall back gracefully.
        self.text_calls.append((system, user))
        return self.text_replies.pop(0) if self.text_replies else ""
