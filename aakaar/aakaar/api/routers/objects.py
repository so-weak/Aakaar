...
references it by `aakaar://t/{tenant_id}/...` in the prompt message. The
chat UI needs to display the image inline; browsers don't speak the
`aakaar://` scheme. This endpoint translates a managed-storage URI to its
...
from aakaar.api.deps import get_object_store, require_tenant_user
...
_URI_RE = re.compile(r"^aakaar://t/([0-9a-f-]{36})/(.+)$")
...
def get_object(
    ...
    if match is None:
        raise HTTPException(status_code=400, detail="invalid aakaar:// uri")
