"""Central logging configuration for the Aakar backend.
...
named logger via `logging.getLogger(__name__)` (e.g. `aakar.api.routers.runs`)
and call into it without thinking about formatters or handlers.

Environment variables
---------------------
- AAKAR_LOG_LEVEL: root level (default INFO). DEBUG | INFO | WARNING | ERROR.
- AAKAR_LOG_FORMAT: "text" (default, human-readable) or "json".
- AAKAR_LOG_LEVELS: comma-separated overrides for specific loggers, e.g.
  "aakar.planner=DEBUG,aakar.workers.browser=DEBUG,sqlalchemy.engine=INFO".
- AAKAR_LOG_NOISY_THIRD_PARTY: "quiet" (default) caps a known list of chatty
...
def setup_logging(*, force: bool = False) -> None:
    ...
    level_name = os.environ.get("AAKAR_LOG_LEVEL", "INFO").upper()
    level = getattr(logging, level_name, logging.INFO)

    fmt = os.environ.get("AAKAR_LOG_FORMAT", "text").lower()
    ...
    # Aakar's own loggers always emit at the configured root level. We set
    # this explicitly so a stray `logging.basicConfig()` somewhere upstream
    # (e.g. uvicorn) can't silently raise our level.
    logging.getLogger("aakar").setLevel(level)

    if os.environ.get("AAKAR_LOG_NOISY_THIRD_PARTY", "quiet").lower() != "verbose":
        for name in _NOISY_LOGGERS:
            logging.getLogger(name).setLevel(logging.WARNING)

    overrides = os.environ.get("AAKAR_LOG_LEVELS", "")
    if overrides:
        for chunk in overrides.split(","):
            ...
            try:
                ...
                logging.getLogger(__name__).warning(
                    "ignoring invalid AAKAR_LOG_LEVELS entry: %r", chunk
                )
