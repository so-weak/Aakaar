...
class LLMClient(Protocol):
    ...
    `complete_planner` is the single entry point: send the conversation,
    constrain the response to `PlannerCompletion`, return a parsed object.
    Any provider-specific knobs (model, temperature) are bound at construction
    time, not per-call — the planner has one job and shouldn't be tuning the
    model on the fly.
    """
...
@dataclass
class FakeLLMClient:
    """Test double. `replies` is a queue of pre-baked completions; each call
    pops the next one. If the queue is exhausted the client raises.

    Records call history in `calls` so tests can assert on what was sent.
    """
