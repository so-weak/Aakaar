...
Reads the bytes for an `aakar://` URI from the tenant's object store
...
class _Inputs(BaseModel):
    ...
    file_uri: str = Field(
        description=(
            "Managed-storage URI (aakar://...) of the source file. Use upstream "
...
# Mirrors aakar/capabilities/file_upload — recover the user-facing
...
async def handler(ctx: ActivityContext, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if not file_uri.startswith("aakar://"):
        raise ValueError(
            f"cap.sftp_write: file_uri must start with 'aakar://', got {file_uri!r}"
        )
