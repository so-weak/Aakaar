...
returns the URI. Gated behind `AAKAR_ALLOW_LOCAL_PATHS=true` because a
...
async def read_local(ctx: ActivityContext, inputs: dict[str, Any]) -> dict[str, Any]:
    """Copy a local file into managed storage and return its `aakar://` URI.
    ...
    — set AAKAR_ALLOW_LOCAL_PATHS=true to enable on dev hosts where the
    API and the user share a filesystem.
    """
    if os.environ.get("AAKAR_ALLOW_LOCAL_PATHS", "false").lower() not in (
        "1",
        "true",
        "yes",
    ):
        logger.warning(
            "file.read_local denied: AAKAR_ALLOW_LOCAL_PATHS not enabled (run_id=%s)", ctx.run_id
        )
        raise PermissionError(
            "file.read_local is disabled; set AAKAR_ALLOW_LOCAL_PATHS=true on "
